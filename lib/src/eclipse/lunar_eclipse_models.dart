...
enum EclipseKind {
  ...
  const EclipseKind(this.mask);

  /// Bit used by the Taiyin C ABI.
  final int mask;

  static Set<EclipseKind> fromMask(int mask) {
...
enum LunarEclipseContact {
  ...
  const LunarEclipseContact(this.nativeIndex);
...
enum LunarEclipseSolveOption {
  ...
  const LunarEclipseSolveOption(this.mask);

  final int mask;
}

/// Options for finding lunar eclipses.
enum LunarEclipseSearchOption {
...
enum LocalLunarEclipseVisibilityFlag {
  ...
  const LocalLunarEclipseVisibilityFlag(this.mask);

  final int mask;

  static Set<LocalLunarEclipseVisibilityFlag> fromMask(int mask) {
    return Set.unmodifiable(values.where((value) => (mask & value.mask) != 0));
  }
}

/// One local lunar-eclipse contact and the Moon's horizontal coordinates.
final class LocalLunarEclipseContact<S extends TimeScale> {
  const LocalLunarEclipseContact({
...
final class LunarEclipseResult<S extends TimeScale> {
  LunarEclipseResult({
    ...
    required Map<LunarEclipseContact, JulianDate<S>?> contacts,
  ...
  final Map<LunarEclipseContact, JulianDate<S>?> contacts;
...
final class LocalLunarEclipseResult<S extends TimeScale> {
  LocalLunarEclipseResult({
    required Set<EclipseKind> kinds,
    required Set<LocalLunarEclipseVisibilityFlag> visibility,
    required this.maximum,
    required this.deltaTSeconds,
    required this.umbralMagnitude,
    required this.penumbralMagnitude,
    required Map<LunarEclipseContact, LocalLunarEclipseContact<S>?> contacts,
  ...
  final Set<EclipseKind> kinds;
  final Set<LocalLunarEclipseVisibilityFlag> visibility;
  final JulianDate<S>? maximum;
  final double? deltaTSeconds;
  final double? umbralMagnitude;
  final double? penumbralMagnitude;
  final Map<LunarEclipseContact, LocalLunarEclipseContact<S>?> contacts;
