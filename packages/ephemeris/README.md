...
> **Pre-release:** the current package line is `1.0.0-beta.10`. Public APIs
...
```dart
print(ephemeris.libraryVersion);  // 1.0.0-beta.10 with this release's native build
...
the FFI boundary end to end: the ABI-11 native entry points use
...
Native ABI-11 calculations return an immutable named record:
...
This package requires an ABI-11 native library that reports the
...
Chinese-calendar, and Ganzhi-rule symbols. Incomplete ABI-11 builds are rejected
...
Native integration tests use three pinned modular ABI-11 libraries: core under
