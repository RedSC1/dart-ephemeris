...
Directory _taiyinPackageRoot() {
  ...
  final workspacePackage = Directory('${current.path}/packages/ephemeris');
  if (File(
...
void main() {
  group('native compatibility', () {
    test('accepts ABI 11 with split-time and Chinese-calendar support', () {
      expect(
    ...
    test(
      'rejects an ABI-11 library without the Chinese-calendar capability',
      () {
        expect(
          () => validateTaiyinNativeCompatibility(
            abiVersion: taiyinSupportedAbiVersion,
    ...
    test('rejects an ABI-11 library without split-time support', () {
      expect(
        () => validateTaiyinNativeCompatibility(
          abiVersion: taiyinSupportedAbiVersion,
          capabilities: 0,
    ...
    test('accepts a library that exposes every required ABI-11 symbol', () {
      expect(
    ...
    test('required ABI-11 set covers every directly invoked core symbol', () {
      final missing =
    ...
    test('reports missing ABI-11 symbols before lazy lookup', () {
      expect(
        () => validateTaiyinRequiredSymbols(
