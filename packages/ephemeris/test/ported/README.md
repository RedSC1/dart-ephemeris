...
- `c_api_cpp`: ABI version, exact library version, typed capabilities, and
  initialized calendar behavior are covered by `test/ephemeris_test.dart`.
...
- `c_api` and `c_api_static`: metadata, runtime/context lifetime, calendar,
  ...
  routes. `ephemeris_test.dart` covers process-wide custom position/state
  ...
  fitted-model extrapolation, Dart-side flag contracts, and custom
  ayanamsha/house-system callbacks including fallback, clear, close, and
  runtime-reset lifecycle behavior. `runtime_api_test.dart` covers registration
  of built-in astrology targets for ordinary position routes.
