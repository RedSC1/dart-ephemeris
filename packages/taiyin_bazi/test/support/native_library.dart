...
///
/// Defaults to the pinned copy bundled in the core package's `lib/native/`
/// (full-module build: Chinese calendar + Ganzhi + BaZi + Ziwei). Override
/// with the `TAIYIN_TEST_LIBRARY` environment variable to test against a
/// freshly built library.
String get libraryPath =>
    Platform.environment['TAIYIN_TEST_LIBRARY'] ??
    '../../lib/native/libtaiyin.dylib';

/// Whether [libraryPath] exists on disk.
bool get nativeLibraryAvailable => File(libraryPath).existsSync();

/// ABI-8 baseline library (Chinese calendar only; no BaZi/Ziwei extension),
