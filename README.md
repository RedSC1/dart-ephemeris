...
- `taiyin` (repository root) — the core ephemeris: runtime, time, positions,
  ...
  automatically.
- [`taiyin_bazi`](packages/taiyin_bazi) — the optional BaZi (八字) extension.
  Importing it adds `context.bazi` / `context.createBazi()` to
  `EphemerisContext`.
- [`taiyin_ziwei`](packages/taiyin_ziwei) — the optional Ziwei Doushu (紫微斗数)
...
the core package; `packages/taiyin_bazi` and `packages/taiyin_ziwei` have their
own suites).
